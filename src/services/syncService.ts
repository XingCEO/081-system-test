...
export async function exportAllData(): Promise<string> {
  ...
  const data = {
    ...
    modifierRecipes: await db.modifierRecipes.toArray(),
...
export async function importAllData(jsonString: string): Promise<void> {
  ...
  try {
    ...
    throw new Error('JSON 格式無效，請確認檔案內容');
  }
  ...
  const modifierRecipes = asArray<ModifierRecipeItem>(data.modifierRecipes);
  ...
  await db.transaction(
    ...
    async () => {
      ...
      await db.modifierRecipes.clear();
      ...
      if (productRecipes.length) await db.productRecipes.bulkPut(productRecipes);
      if (modifierGroups.length) await db.modifierGroups.bulkPut(modifierGroups);
      if (modifiers.length) await db.modifiers.bulkPut(modifiers);
      if (modifierRecipes.length) await db.modifierRecipes.bulkPut(modifierRecipes);
      if (tables.length) await db.diningTables.bulkPut(tables);
      if (orders.length) await db.orders.bulkPut(orders);
      if (orderItems.length) await db.orderItems.bulkPut(orderItems);
      ...
      if (inventoryTransactions.length) await db.inventoryTransactions.bulkPut(inventoryTransactions);
      if (dailySummaries.length) await db.dailySummaries.bulkPut(dailySummaries);
...
export async function resetAllData(): Promise<void> {
  try {
    ...
    // If API fails, only clear local tables instead of deleting the entire database
    await db.transaction('rw',
      ...
      async () => {
        await Promise.all([
          db.categories.clear(), db.products.clear(), db.ingredients.clear(),
          db.productRecipes.clear(), db.modifierGroups.clear(), db.modifiers.clear(),
          db.diningTables.clear(), db.orders.clear(), db.orderItems.clear(),
          db.employees.clear(), db.shifts.clear(), db.inventory.clear(),
          db.inventoryTransactions.clear(), db.dailySummaries.clear(), db.settings.clear(),
        ]);
      }
    );
  }
