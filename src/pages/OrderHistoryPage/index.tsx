  ...
  return (
    <div className="h-full flex flex-col">
      <div className="page-header">
        <h1 className="text-xl font-bold text-gray-800 dark:text-white flex items-center gap-2"><IconClipboard className="w-6 h-6 text-indigo-500" /> 訂單記錄</h1>
            ...
            {orders.map((order, i) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-gray-400 dark:text-gray-500 hidden sm:inline">{formatDateTime(order.createdAt)}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        >
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div>
                <span className="text-gray-500 dark:text-gray-400">桌位</span>
                <p className="font-medium text-gray-800 dark:text-white">{selectedOrder.order.tableName}</p>
              </div>
              <div>
                <span className="text-gray-500 dark:text-gray-400">經手人</span>
                <p className="font-medium text-gray-800 dark:text-white">{selectedOrder.order.employeeName}</p>
              </div>
              <div>
                <span className="text-gray-500 dark:text-gray-400">時間</span>
                <p className="font-medium text-gray-800 dark:text-white">{formatDateTime(selectedOrder.order.createdAt)}</p>
              </div>
            </div>

            <div className="border-t border-gray-200 dark:border-gray-700 pt-4">
              <h3 className="font-semibold mb-3 text-gray-800 dark:text-white">訂單明細</h3>
              <div className="space-y-2">
                {selectedOrder.items.map((item) => (
                  <div key={item.id} className="flex justify-between">
                    <div>
                      <span className="font-medium text-gray-800 dark:text-white">{item.quantity}x {item.productName}</span>
                    ...
                    <span className="font-medium text-gray-800 dark:text-white">{formatPrice(item.subtotal)}</span>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="border-t border-gray-200 dark:border-gray-700 pt-4 space-y-1">
              <div className="flex justify-between font-bold text-lg">
                <span className="text-gray-800 dark:text-white">總計</span>
