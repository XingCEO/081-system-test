    ...
    saveTimeoutRef.current = window.setTimeout(() => {
      void (async () => {
        ...
        try {
          ...
          if (table) {
            await updateTable(tableId, { x: table.x, y: table.y });
          }
        } catch {
          toast.error('儲存桌位位置失敗');
        } finally {
          ...
          {isAdmin && (
            <>
              ...
              <button
                ...
                disabled={isSaving}
                className={`${editMode ? 'btn-primary' : 'btn-secondary'} ${isSaving ? 'opacity-50 pointer-events-none' : ''}`}
              >
                {isSaving
                  ? '儲存中...'
                  : editMode
                    ? <><IconCheck className="w-4 h-4 inline" /> 完成編輯</>
                    : <><IconPencilSquare className="w-4 h-4 inline" /> 編輯佈局</>}
              </button>
            </>
          )}
        </div>
