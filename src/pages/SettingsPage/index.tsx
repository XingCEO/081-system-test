  ...
  return (
    <div className="h-full overflow-auto">
      <div className="max-w-3xl mx-auto p-6 space-y-8">
        <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><IconSettings className="w-6 h-6 text-slate-500" /> 系統設定</h1>

        {/* Store Info */}
        <div className="card p-6 space-y-4">
          <h2 className="text-lg font-semibold text-slate-800 flex items-center gap-2"><IconStorefront className="w-5 h-5" /> 店家資訊</h2>
          <div>
            <label className="text-sm font-medium text-slate-700 block mb-1">店家名稱</label>
            <input value={(values.storeName as string) || ''} onChange={e => updateSetting('storeName', e.target.value)} className="input-field" />
          </div>
          <div>
            <label className="text-sm font-medium text-slate-700 block mb-1">地址</label>
            <input value={(values.storeAddress as string) || ''} onChange={e => updateSetting('storeAddress', e.target.value)} className="input-field" />
          </div>
          <div>
            <label className="text-sm font-medium text-slate-700 block mb-1">電話</label>
        ...
        {/* Receipt */}
        <div className="card p-6 space-y-4">
          <h2 className="text-lg font-semibold text-slate-800 flex items-center gap-2"><IconReceipt className="w-5 h-5" /> 收據設定</h2>
          <div>
            <label className="text-sm font-medium text-slate-700 block mb-1">收據標頭（額外文字）</label>
          ...
          <div>
            <label className="text-sm font-medium text-slate-700 block mb-1">收據頁尾</label>
            <input value={(values.receiptFooter as string) || ''} onChange={e => updateSetting('receiptFooter', e.target.value)} className="input-field" />
          </div>
          <div>
            <label className="text-sm font-medium text-slate-700 block mb-1">貨幣符號</label>
        ...
        {/* General */}
        <div className="card p-6 space-y-4">
          <h2 className="text-lg font-semibold text-slate-800 flex items-center gap-2"><IconWrench className="w-5 h-5" /> 一般設定</h2>
          <div>
            <label className="text-sm font-medium text-slate-700 block mb-1">自動登出時間 (分鐘)</label>
          ...
          <div>
            <label className="text-sm font-medium text-slate-700 block mb-1">低庫存預設閾值</label>
          ...
          <div className="flex items-center gap-3">
            ...
            <label htmlFor="enableSound" className="text-sm font-medium text-slate-700">啟用音效提醒</label>
          </div>
        </div>
        ...
        {/* Data Management */}
        <div className="card p-6 space-y-4">
          <h2 className="text-lg font-semibold text-slate-800 flex items-center gap-2"><IconSave className="w-5 h-5" /> 資料管理</h2>
        ...
        <div className="card p-6 border-red-200 space-y-4">
          <h2 className="text-lg font-semibold text-red-600 flex items-center gap-2"><IconWarning className="w-5 h-5" /> 危險區域</h2>
          <p className="text-sm text-slate-500">重置將清除所有資料，包括訂單、菜單、員工等。此操作無法復原。</p>
