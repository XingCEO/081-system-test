...
export default function InventoryPage() {
  ...
  const getStatusBadge = (inv: InventoryRecord) => {
    if (inv.currentStock <= 0) return <span className="bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-400 px-2 py-0.5 rounded-full text-xs font-medium">售完</span>;
    if (inv.currentStock <= inv.lowStockThreshold) return <span className="bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-400 px-2 py-0.5 rounded-full text-xs font-medium">偏低</span>;
    return <span className="bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-400 px-2 py-0.5 rounded-full text-xs font-medium">正常</span>;
  };

  return (
    <div className="h-full flex flex-col">
      <div className="page-header">
        <h1 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2"><IconPackage className="w-6 h-6 text-amber-500" /> 庫存管理</h1>
      ...
      <div className="flex-1 overflow-auto p-4">
        {!filtered?.length ? (
          <div className="text-center py-16 text-slate-400 dark:text-slate-600 animate-fade-in">
            ...
            <p className="text-lg font-medium">沒有符合的庫存項目</p>
          </div>
        ) : (
          <div className="space-y-2">
            {filtered.map((inv, i) => (
              <div key={inv.id} className={`card px-4 py-3 flex items-center justify-between animate-slide-up stagger-${Math.min(i + 1, 6)}`}>
                <div className="flex items-center gap-4">
                  <div className="text-center min-w-[60px]">
                    <p className={`text-2xl font-bold ${inv.currentStock <= 0 ? 'text-red-600 dark:text-red-400' : inv.currentStock <= inv.lowStockThreshold ? 'text-amber-600 dark:text-amber-400' : 'text-emerald-600 dark:text-emerald-400'}`}>
                      {inv.currentStock}
                    </p>
                    <p className="text-xs text-slate-400 dark:text-slate-500">{inv.unit}</p>
                  </div>
                  <div>
                    <h3 className="font-semibold text-slate-900 dark:text-white">{inv.productName}</h3>
                    <p className="text-sm text-slate-500 dark:text-slate-400">閾值：{inv.lowStockThreshold} {inv.unit}</p>
                  </div>
                  {getStatusBadge(inv)}
                </div>
        ...
        <Modal open={true} onClose={() => setActionModal(null)} title={`${actionModal.action === 'restock' ? '進貨' : actionModal.action === 'adjust' ? '調整庫存' : '報廢'} - ${actionModal.record.productName}`} size="sm">
          <div className="space-y-4">
            <div className="text-center bg-slate-50 dark:bg-slate-800 rounded-xl p-4">
              ...
              <p className="text-3xl font-bold text-slate-900 dark:text-white">{actionModal.record.currentStock} {actionModal.record.unit}</p>
            </div>
            <div>
      ...
      {historyModal && (
        <Modal open={true} onClose={() => setHistoryModal(null)} title={`${historyModal.productName} - 異動記錄`} size="lg">
          <div className="space-y-2 max-h-[60vh] overflow-auto">
            {history.length === 0 ? (
              <p className="text-center text-slate-400 dark:text-slate-600 py-8">尚無記錄</p>
            ) : (
              history.map((tx) => (
                <div key={tx.id} className="flex items-center justify-between py-2 border-b border-slate-100 dark:border-slate-800">
                  <div className="flex items-center gap-3">
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                      tx.type === 'sale' ? 'bg-red-100 text-red-700 dark:bg-red-900/50 dark:text-red-400' :
                      tx.type === 'restock' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/50 dark:text-emerald-400' :
                      tx.type === 'waste' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/50 dark:text-amber-400' :
                      'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-400'
                    }`}>
                      {INVENTORY_TRANSACTION_LABELS[tx.type]}
                    </span>
                    <div>
                      <span className={`font-semibold ${tx.quantity > 0 ? 'text-emerald-600 dark:text-emerald-400' : 'text-red-600 dark:text-red-400'}`}>
                        {tx.quantity > 0 ? '+' : ''}{tx.quantity}
                      </span>
                      <span className="text-sm text-slate-500 dark:text-slate-400 ml-2">{tx.previousStock} → {tx.newStock}</span>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-xs text-slate-400 dark:text-slate-500">{formatDateTime(tx.createdAt)}</p>
