...
function getSyncExportData() {
  return {
    ...
    employees: db.prepare('SELECT * FROM employees').all().map(r => rowToJs(r as Record<string, unknown>)),
...
function getOrderIngredientUsage(items: Array<Record<string, unknown>>): IngredientUsage[] {
  const usages = items.flatMap((item) => {
    ...
    return getProductIngredientUsage(Number(item.productId), quantity);
  });
...
function importSyncData(
  ...
  const tablesToClear = options.menuOnly
    ? ['inventory', 'product_recipes', 'ingredients', 'modifiers', 'modifier_groups', 'products', 'categories']
    : ['settings', 'inventory_transactions', 'inventory', 'shifts', 'employees',
        'order_items', 'orders', 'dining_tables', 'modifiers', 'modifier_groups',
  ...
  db.transaction(() => {
    ...
    if (data.ingredients) insertWithId('ingredients', ['id', 'name', 'unit', 'costPerUnit', 'lowStockThreshold', 'isActive', 'sortOrder', 'createdAt', 'updatedAt'], data.ingredients as Array<Record<string, unknown>>);
...
app.put('/api/categories/:id', (req, res) => {
  ...
  ).run(data.name, data.description, data.sortOrder, data.isActive, data.icon, data.color, now, req.params.id);
...
app.put('/api/products/:id', (req, res) => {
  ...
  db.prepare(
    ...
    data.categoryId, data.name, data.description, data.price, data.imageUrl,
    data.isActive, data.modifierGroupIds, data.trackInventory,
    data.sortOrder, data.isCombo, data.comboItems, now, req.params.id
  );
...
app.get('/api/orders/:id', (req, res) => {
  ...
  const items = db.prepare('SELECT * FROM order_items WHERE orderId = ?')
    .all(req.params.id).map(r => rowToJs(r as Record<string, unknown>));
...
app.put('/api/tables/:id', (req, res) => {
  ...
  db.prepare(
    ...
    data.shape, data.status, data.currentOrderId, data.floor, data.isActive, req.params.id
  );
...
app.post('/api/ingredients', (req, res) => {
  ...
  ).run(data.name, data.unit ?? '份', data.costPerUnit ?? 0, data.lowStockThreshold ?? 10, count + 1, now, now);

  const ingId = Number(result.lastInsertRowid);
  db.prepare(
    'INSERT INTO inventory (ingredientId, ingredientName, currentStock, lowStockThreshold, unit, lastUpdated) VALUES (?, ?, 0, ?, ?, ?)'
  ).run(ingId, data.name, data.lowStockThreshold ?? 10, data.unit ?? '份', now);
  db.prepare('UPDATE inventory SET currentStock = ? WHERE ingredientId = ?').run(currentStock, ingId);
...
app.put('/api/ingredients/:id', (req, res) => {
  ...
  const id = Number(req.params.id);
  db.prepare(
    'UPDATE ingredients SET name=?, unit=?, costPerUnit=?, lowStockThreshold=?, isActive=?, updatedAt=? WHERE id=?'
  ).run(data.name, data.unit, data.costPerUnit, data.lowStockThreshold, data.isActive ?? 1, now, id);
...
app.get('/api/recipes/:productId', (req, res) => {
  const rows = db.prepare('SELECT * FROM product_recipes WHERE productId = ?')
    .all(req.params.productId).map(r => rowToJs(r as Record<string, unknown>));
  res.json(rows);
});
