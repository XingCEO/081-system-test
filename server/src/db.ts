...
db.exec(`
  ...
  CREATE INDEX IF NOT EXISTS idx_modifier_recipes_modifier ON modifier_recipes(modifierId);
...
function addIngredientsColumnIfMissing(sql: string): void {
  try {
    db.exec(sql);
  } catch (error) {
...
addIngredientsColumnIfMissing("ALTER TABLE ingredients ADD COLUMN notes TEXT DEFAULT '';");
